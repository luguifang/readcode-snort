...
SnortConfig * ParseSnortConf(void)
{
    ...
    //鏋勯?犻粯璁ょ殑鐢ㄦ埛鑷畾涔夌殑瑙勫垯绫诲瀷琛? 涓昏濉厖RuleListNode------lgf
 ...
 *
 ***************************************************************************/
#if 0  // avoid warning: 閳ユェumpList閳??defined but not used
 ...
 * Finish adding the rule to the port tables
   完成将规则添加到端口列表中：大致步骤
   1、找到该规则属于哪张表(src/dst/any-any tcp,udp,icmp,ip or nocontent)
   2、为sid:gid组合找到一个索引,sid:gid标志了规则的唯一性
   3、添加所有的没有content关键字的规则到单独的没有content关键字的端口对象中,无关紧要的端口让他成为any-any端口对象
   4、如果是一个any-any的规则那么将他添加到 any-any端口对象中
   5、查找我们是否拥有了我们定义的端口对象，如果没有则创建它：
   		a、为源端口、目的端口执行此操作
   		b、为端口对象中的规则添加索引
   		c、如果规则是双向的则源端口、目的端口表中都需要添加
 *
                              ...
                              int proto, port_entry_t *pe, FastPatternConfig *fp)
{
     ...
     * were using a single rule group we make it an any-any rule. */
     /*
		如果any-any规则将规则索引添加到any-any端口对象中，如果是any-any端口规则有没有content关键字都会走这儿
		如果我们拥有一个any-any 规则或者一个大端口组 或者我们使用了一个单独的规则组，那么我们让它成为一个any-any规则
	*/
        ...
        large_port_group || fpDetectGetSingleRuleGroup(fp))
    {
        if (proto == ETHERNET_TYPE_IP)
        {
             ...
             * IP table */
             /*
             如果我们应用了这些协议那么将ip规则添加到教高的应用协议组中。所有的ip规则应该拥有一个any-any端口描述
             并且加入这个测试中。没有tcp/udp/icmp的ip规则只会进入到ip表中
			*/
            DEBUG_WRAP(DebugMessage(DEBUG_PORTLISTS,
    ...
    if (!(rtn->flags & ANY_DST_PORT))
    {
		...
		//将规则添加到了端口对象PortObject的rule_list成员中，形成了基本的 端口列表-->端口对象-->规则。
...
static int ProcessIP(SnortConfig *sc, char *addr, RuleTreeNode *rtn, int mode, int neg_list)
{
    ...
    if(mode == SRC)
    {
        ...
        if (rtn->sip == NULL)
        {
            ...
            {
            	/*不存在的花需要把rtn->sip 加入到ip_vartable中去*/
...
static void ParseConfigFile(SnortConfig *sc, SnortPolicy *p, char *fname)
{
    ...
    /* open the rules file 打开include中的配置规则文件 */
    if (fp == NULL)
    {
...
/*
解析规则alert/drop/...等规则类型之后的部分
根据规则类型是alert/drop/...等将解析后的规则安插到总的配置sc的Alert、Log、等列表中
*/
                      ...
                      RuleType rule_type, ListHead *list)
{
	...
	// 和ip地址形式一致只是取法不同，都会在每个策略中维护一个总的变量表
    ...
    else
    {
		...
		/*ip地址的组织形式：
		该策略的每个规则的地址会加该策略的ip_vartable中 形态：
    ...
    port_list_add_entry(&port_list, &pe);//加到全局的port_list中
     ...
     */
    /*形成通过端口寻找关联规则，可以参看函数描述-----需重点关注的内容*/
	//port_tables 核心端口变
                                      ...
                                      ListHead *list)
{
    ...
    /*
    	如果没有匹配到任何已经存在的节点，那么就会创建新的节点，并把器安插到list尾部
	*/
    if (rtn == NULL)
    {
